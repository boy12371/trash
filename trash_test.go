...
func TestListPackages(t *testing.T) {
	assert := require.New(t)
	p := listPackages("github.com/boy12371/trash", "vendor")
	logrus.Debug(p)
	assert.Equal(3, len(p))
	assert.Contains(p, "github.com/boy12371/trash")
	assert.Contains(p, "github.com/boy12371/trash/util")
	assert.Contains(p, "github.com/boy12371/trash/conf")
}
